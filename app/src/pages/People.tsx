...
import wordsen from '../i18n/resources/en/pages/people.json';
import wordszhTW from '../i18n/resources/zh-TW/pages/people.json';
import wordsko from '../i18n/resources/ko/pages/people.json';
import { Link } from 'react-router-dom';
import SiteLayout from '../components/SiteLayout';
import { localize, useLocale } from '../lib/lang';
import useLogic from '../logic/People';
import './People.css';

/** People — one component for every language, generated by
 *  scripts/convert-pages.mjs from People.EN.dc.html and its KO twin. The words live in
 *  src/i18n/resources/<locale>/pages/people.json; the
 *  line-heights that differ by language are custom properties in People.css.
 *  Edit the legacy pages, not this file. */
export default function People() {
  const t = usePageWords("people", { "en": wordsen, "zh-TW": wordszhTW, "ko": wordsko });
  const locale = useLocale();
  const { closedToNew, openToNew, roster } = useLogic();

  return (
    <SiteLayout page="People.EN.dc.html" className="page-people" footer={{ loop: "0.103", cta: "#46325A" }}>
      <div style={{ fontFamily: "var(--font-serif)", minHeight: "100vh", overflowX: "hidden" }}>
        ...
        <section style={{ background: "var(--color-plum)", color: "var(--color-paper)" }}>
          <div style={{ maxWidth: "1320px", margin: "0 auto", padding: "52px 28px 60px" }}>
            ...
            <h1 style={{ fontFamily: "var(--font-serif)", fontWeight: "600", fontSize: "clamp(40px, 5.6vw, 80px)", lineHeight: "var(--in-people-0)", letterSpacing: "-0.02em", margin: "0 0 22px", maxWidth: "18ch", textWrap: "balance" }}>
              {t("001_h1")}
            </h1>
        ...
        <section style={{ maxWidth: "1320px", margin: "80px auto 0", padding: "0 28px" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-serif)", fontWeight: "600", fontSize: "clamp(30px, 4vw, 52px)", lineHeight: "var(--in-people-1)", letterSpacing: "-0.01em", margin: "0 0 24px", textWrap: "balance" }}>
            {t("004_h2")}
          </h2>
        ...
        <section style={{ maxWidth: "1320px", margin: "80px auto 0", padding: "0 28px" }}>
          <div style={{ borderTop: "1px solid rgba(38,42,56,0.14)", paddingTop: "44px" }}>
            <p style={{ fontFamily: "var(--font-serif)", fontStyle: "var(--in-people-2)", fontWeight: "400", fontSize: "var(--in-people-3)", lineHeight: "var(--in-people-4)", width: "800px", maxWidth: "100%", margin: "0", color: "#43326D" }}>
              {t("007_p")}
            </p>
            <div style={{ fontFamily: "var(--in-people-5)", fontStyle: "italic", fontWeight: "var(--in-people-6)", fontSize: "var(--in-people-7)", lineHeight: "1.5", marginTop: "20px", color: "var(--in-people-8)" }}>
              {t("008_div")}
            </div>
          </div>
        </section>
        ...
        <section id="join" style={{ background: "var(--color-plum)", color: "var(--color-paper)", marginTop: "80px" }}>
          <div style={{ maxWidth: "1320px", margin: "0 auto", padding: "70px 28px", textAlign: "center" }}>
            ...
            <h2 style={{ fontFamily: "var(--font-serif)", fontWeight: "500", fontSize: "clamp(32px, 4.6vw, 60px)", lineHeight: "var(--in-people-9)", margin: "0 auto 18px", maxWidth: "18ch", letterSpacing: "-0.01em", textWrap: "balance" }}>
              {t("010_h2")}
            </h2>
            {openToNew ? (
              <>
